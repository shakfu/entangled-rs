...
impl ConfigUpdate {
    ...
    pub fn merge_into(&self, base: &Config) -> Config {
        Config {
            ...
            markers: self.markers.clone().unwrap_or_else(|| base.markers.clone()),
            languages: merge_languages(
                &base.languages,
                self.languages.as_ref().unwrap_or(&Vec::new()),
            ),
            watch: self.watch.clone().unwrap_or_else(|| base.watch.clone()),
...
fn merge_hooks(base: &HooksConfig, update: Option<&HooksConfig>) -> HooksConfig {
    match update {
        Some(u) => HooksConfig {
            ...
            quarto_attributes: u.quarto_attributes || base.quarto_attributes,
            build: u.build.clone().or_else(|| base.build.clone()),
            brei: u.brei.clone().or_else(|| base.brei.clone()),
        },
        None => base.clone(),
    }
}
